local function do_keyboard_endchat()
    ...
    keyboard.inline_keyboard = {
    	{
    		{text = '🔚 Back To Menu 🔙', callback_data = '/end'}
	    }
    }
    return keyboard
end
...
api.forwardMessage('-115722023', msg.chat.id, msg_id) 
api.sendKeyboard(msg.chat.id, '`- Sent !`\n_Please wait for reply_'  ,do_keyboard_endchat(), true)
end
if blocks[1] == 'block' then
if msg.reply and msg.reply.forward_from and msg.chat.type == 'group' and msg.chat.id == -115722023 and not blocks[2] then
msg = msg.reply
...
api.sendMessage(user_id, '`- Sorry`\n_You are Blocked_\n\n*Chat ended.*', true) 
 ...
 if msg.chat.type == 'group' and msg.chat.id == -115722023 then
...
api.sendMessage(user_id, '_You are UnBlocked_', true) 
    ...
    if msg.reply and msg.reply.forward_from and msg.chat.type == 'group' and msg.chat.id == -115722023 then
